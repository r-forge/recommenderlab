...
REAL_PREA_AVG<- function(data, parameter= NULL) {
  ...
  model <- c(list(description = "PREA: AVG", preaObject = NULL, data=data), param)
  ...
  if(param$lazy == FALSE) {
    ...
    model <- c(list(description = "PREA: AVG", preaObject = recommenderJObject, data=data), param)
  ...
  new("Recommender", method = "PREA_AVG", dataType = class(data),
