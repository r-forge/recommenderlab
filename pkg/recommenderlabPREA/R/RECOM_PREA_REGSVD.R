...
REAL_PREA_REGSVD<- function(data, parameter= NULL) {
  ...
  model <- c(list(description = "PREA: REGSVD", preaObject = NULL, data=data), param)
  ...
  if(param$lazy == FALSE) {
    ...
    strings <- .jarray( c(param$method, "simple", "0.2", param$feature_count,
                          param$learning_rate, param$regularizer, param$max_iteration))
    recommenderJObject <- .jcall(interface, returnSig = "LRecContainer;","createRecommender", ratingMat, strings)
    model <- c(list(description = "PREA: REGSVD", preaObject = recommenderJObject, data=data), param)
  ...
  predict <- function(model, newdata, n = 10, data=NULL, type=c("topNList", "ratings"), ...) {
    ...
    if(class(newdata) == "integer") {
      print("integer")
      if(model$lazy == TRUE) {
        strings <- .jarray( c(model$method, "simple", "0.2", model$feature_count,
                              model$learning_rate, model$regularizer, model$max_iteration))
        recommenderJObject <- .jcall(interface, returnSig = "LRecContainer;","createRecommender", ratingMat, strings)        
