...
REAL_PREA_ITEMAVG<- function(data, parameter= NULL) {
  
  param <- .get_parameters(.REAL_PREA_ITEMAVG_PARAM, parameter)
  ...
  model <- c(list(description = "PREA: ITEMAVG", preaObject = NULL, data=data), param)
  ...
  if(param$lazy == FALSE) {
    ...
    model <- c(list(description = "PREA: ITEMAVG", preaObject = recommenderJObject, data=data), param)
  ...
  new("Recommender", method = "PREA_ITEMAVG", dataType = class(data),
