...
REAL_PREA_RANDOM<- function(data, parameter= NULL) {
  
  param <- .get_parameters(.REAL_PREA_RANDOM_PARAM, parameter)
  ...
  model <- c(list(description = "PREA: RANDOM", preaObject = NULL, data=data), param)
  ...
  if(param$lazy == FALSE) {
    ...
    model <- c(list(description = "PREA: RANDOM", preaObject = recommenderJObject, data=data), param)
  ...
  new("Recommender", method = "PREA_RANDOM", dataType = class(data),
