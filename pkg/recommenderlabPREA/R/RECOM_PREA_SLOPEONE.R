...
REAL_PREA_SLOPEONE<- function(data, parameter= NULL) {
  ...
  model <- c(list(description = "PREA: SLOPEONE", preaObject = NULL, data=data), param)
  ...
  if(param$lazy == FALSE) {
    ...
    model <- c(list(description = "PREA: SLOPEONE", preaObject = recommenderJObject, data=data), param)
