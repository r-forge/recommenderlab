 test_code <- function() {
  ...
  data("MovieLense")
     rec <- Recommender(MovieLense, method = "MAHOUT_SVD")
     pre <- predict(rec, 3, n = 10)
     pre
     as(pre, "list")
}

MAHOUT_SVD <- function(data, parameter = NULL) {
...
result <- .jcall(obj,"[I","createRecommender","svdRecommender",data, as.integer(user), as.integer(howmany))
	topN <- new("topNList", items = list(user = result), itemLabels = colnames(model$data), n=length(result))
	topN <- bestN(topN, n) 
	return(topN)
    }

    new("Recommender", method = "MAHOUT_SVD", dataType = class(data),
	    ntrain = nrow(data), model = model, predict = predict)
}

#recommenderRegistry$modify_entry(
recommenderRegistry$set_entry(
	method="MAHOUT_SVD", dataType = "realRatingMatrix", fun= MAHOUT_SVD, 
