...
\abstract{
   \pkg{recommenderlab} provides the infrastructure to test and develop
       ...
       systems for 0-1 data, however in the future it can be extended to 
       also the more thourougly researched case of real-valued rating data.
    }


<<echo=FALSE>>=
options(scipen=3, digits=4, prompt="R> ", eps=FALSE, width=60)
### for sampling
set.seed(1234)
@
...
(\url{http://www.netflixprize.com/}), a challenge to improve 
...
items as the basis of predicting missing ratings and/or for creating top-$N$
...
To reduce the sensitivity, \cite{Deshpande:2004} propose a
...
in $\set{R}$.
\item Recommend $N$ unique right-hand-sides ($Y$) of the marching rules 
...
A method used in the literature to compare two classifiers at different 
...
\caption{UML class diagram for package \pkg{recommenderlab}.}
...
areas/items each user user visited, we can produce a histogram.
...
with the a Given 3 protocol, i.e.,
...
false positive rate (FPR). The plot 
using plot type both and annotated with the size of the top-$N$ list
...
algorithms <- list(
        ...
        "association rules" = list(name="AR", param=list(supp=0.001, conf=0.5, maxlen=2))
        )
...
plot(results, pch=1:5, annotate=c(1,3), legend="right")
@
\begin{figure}
\centerline{\includegraphics[scale=1]{recommenderlab-roc2}}
\caption{Comparison of ROC curves for several recommender methods.}
\label{fig:roc2}
\end{figure}

<<precrec2, fig=TRUE, include=FALSE>>=
plot(results, "prec/rec", pch=1:5, annotate=3)
@
\begin{figure}
...
For this data set and the given evaluation scheme the CF based methods clearly
...
Using automated recommender systems is an important method for online
retailers and for many other applications. But
often no extensive data base of rating data is available and
it makes sense to think about using  0-1 data instead.
...
In this paper we described the R extension package~\pkg{recommenderlab}
...
Adding new recommender algorithms to the package is facilidated using
...
extend to the more conventinal non-binary rating data and its algorithms.
In the future we will add more and more of these algorithms to the package and
we hope that some algorithms will be contributed by other researchers.
