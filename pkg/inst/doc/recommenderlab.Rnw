...
have been developed over the last decade there still is the need for a 
framework which facilitates recommender system research
...
one top-$N$ list for each active user. For ratings \code{n} is ignorred
...
Cercion from data.frame and list into a rating matrix is also provided.
