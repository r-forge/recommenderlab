...
First, we load the package and create a small 
artificial data set as a matrix. 
<<>>=
library("recommenderlab")
data <- matrix(sample(c(0:5, NA), 50, 
...
The realRatingMatrix can be coerced back into a matrix, a list or
a data.frame any time for closer inspection.
<<>>=
as(r, "matrix")
head(as(r, "list"))
...
The list contains one entry for each user (row in the matrix) an each
entry is a vector with only the items for which a rating is available.
The data.frame contains all ratings as a triplet of user, item and rating.
...
Many recommender algorithms can also predict ratings. This is 
