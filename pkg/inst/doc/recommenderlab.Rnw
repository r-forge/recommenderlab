...
\title{Developing and Testing Recommendation Algorithms
with 
\pkg{recommenderlab}\footnote{This research was funded in part by the NSF Industry/University Cooperative Research Center for Net-Centric Software \& Systems.}
}
...
utilized. 
%Although this type of data differs significantly from directly
%elicited ratings, only very limited research is available for
%0-1 data.  
This paper describes \pkg{recommenderlab} which provides the
infrastructure to test and develop recommender algorithms for rating data and 
...
Netflix\footnote{\url{http://www.netflix.com}}, 
iTunes\footnote{\url{http://www.apple.com/itunes/}} and 
...
which rank high for the user's most desireable attributes. 
...
contains a white range of attributes to describe movies including
...
to implement recommender algorithms for colaborative filtering. 
...
collaborative filtering algorithms. Development ended 2005. 
LensKit is a relative new software package with the aim
...
has a completely different goal to the existin software packages. It 
...
The focus is on data handling, 
easy incorporation of algorithms (either implemented in R or
interfacing existing algorithms), 
experiment set up and the evaluation of the results. 

\marginpar{Add: structure}
...
The performance of the recommender algorithm can be improved by removing 
user rating bias.
The first is to normalize the rating data before applying the 
...
by subtracting 
from each rating of a user the user's average rating.
...
An option is to
introduce a weight for each user's ratings into 
Equation~(\ref{equ:aggregation1}) to account for the fact that some users
in the neighborhood are more similar to the active user than others.
...
similarity matrix $\mat{S}$ only $k=3$ largest entries are
...
$$\hat{r}_{ui} = \frac{\sum_{j \in s_i}{s_{ij} r_{uj}}}{\sum_{j \in s_i}{|s_{ij}|}}$$

In Figure~\ref{fig:IBCF} we assume that we have ratings for the  
...
the similarites (only the reduced matrix with the $k=3$
highest ratings is used) and the user's ratings. 
The result (below the matrix) shows that $i_3$ has the highest extimated
...
\subsection{Other colaborative filtering methods}
...
\subsection{Recommendations from 0-1 Data}
...
However, for 0-1 data that would lead
...
Pearson Correaltion (PC)


\subsection{Evaluation Top-$N$ recommenders}

The items in the predicted top-$N$ lists and the 
withheld items  liked by the user (typically determined by a imple threshold
...
Sparse matrices in \pkg{Matrix} typically do not store 0s explicitely,
however for \class{realRatingMatrix} we use these sparse matrices
such that instead of 0s, NAs are not explicitely stored.
...
\section{Examples}
\label{sec:examples}
\subsection{Data handling in recommenderlab}
This fist example shows how to manage data in recommender lab.
First, we load the package. Then and then we create a small 
artificial data set as a matrix and show how to convert it into 
a realRatingMatrix which stores the data in sparse format (only non-NA values
are stored explicitely).
<<>>=
library("recommenderlab")
data <- matrix(sample(c(0:5, NA), 100, 
    replace=TRUE, prob=c(rep(.3/6,6),.7)), ncol=10,
    dimnames=list(user=paste("u", 1:10, sep=''), 
...
Jester5k contains \Sexpr{nrow(Jester5k)} users with 
\Sexpr{nratings(Jester5k)}~ratings for
\Sexpr{ncol(Jester5k)} items. For the following examples we use
...
Next, we want to inspect the ratings for the first user. 
We can inspect the user's ratings
by coercing the first row into a matrix, but here we
use the function
\code{LIST()} which produces for each user a list which
only contains the items for which ratings are available.

<<>>=
LIST(r[1,])
@

We can look at several distributions to understand the data better.
First, we look at the distribution of ratings using a histogram.

<<fig=TRUE>>=
hist(getRatings(r))
@

In the histogram in Fig~\ref{} that the data has an interesting distribution
where all negative values occur with a almost indentical frequency and
