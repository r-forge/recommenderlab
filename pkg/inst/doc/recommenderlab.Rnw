...
Cofi~\footnote{http://www.nongnu.org/cofi/}, 
%CoFE~\footnote{},
RACOFI~\footnote{http://racofi.elg.ca/}, 
SUGGEST~\footnote{http://glaros.dtc.umn.edu/gkhome/suggest/overview/}, 
Vogoo PHP~\footnote{http://www.vogoo-api.com/}).
...
in section~\ref{sec:infrastruction}. In section~\ref{sec:examples} we 
...
$w_{ai}$ is the weigth for user $u_i$ and
$\kappa$ is
...
For 0-1 data we suggest to calculate the following score.
...
items and their similarity values are stored~\citep{Sarwar:2001}.
...
similarity~\citep{Deshpande:2004}. It can be fully precomputed
...
with item withholding (paramter \code{given}). 
...
for 200 randomly choosen users.
